...
{
    protected $modelName = "Acl";
    protected $sectionName = "Acl";
    protected $sectionNamePlural = "Acls";
	...
	{
        $DB = Acl::query();
        $DB->leftJoin('acls as acc', 'acc.id', '=', 'acls.parent_id') 
            ->select('acls.*', 'acc.title as parent_name'); 
		...
		$parent_list 	= 	Acl::get();
		return View("adminpnlx.$this->modelName.index", compact('results', 'searchVariable', 'sortBy', 'order', 'query_string', 'parent_list'));
	}	

	public function create()
	{
		$parent_list =  Acl::where('parent_id', 0)->get();
	...
	public function store(Request $request)
	{
		...
		if (!$SavedResponse) {
			...
			Session()->flash('success', "Acl Module added successfully");
	...
	public function edit($enaclid)
	{
		...
		if (!empty($enaclid)) {
			...
			$modelDetail   =  Acl::find($acl_id);
			// get acls and permission module list
			$modelss =	Acl::with('admin_module_actions')->where('id', $acl_id)->first();
			$parent_list  = Acl::where('parent_id', '!=', $acl_id)->get();
			return  View("adminpnlx.$this->modelName.edit", compact('parent_list', 'acl_id', 'modelss', 'modelDetail'));
		} else {
			return redirect()->route($this->modelName . ".index");
		}
	}
		...
		if (isset($thisData['data']) && !empty($thisData['data'])) {
			foreach ($thisData['data'] as $record) {
				if (!empty($record['name']) && !empty($record['function_name'])) {
					$AclAdminAction	=	AclAction::where('acl_module_id', $acl_id)->where("function_name",$record['function_name'])->first();
					if(empty($AclAdminAction)){
						$obj1 						=  new AclAction;
						$obj1['acl_module_id']	    =  $acl_id;
	...
	public function menu_update(Request $request,$encmsid){
        ...
        if (!empty($encmsid)) {
            ...
            if ($validator->fails()) {
                ...
                Session()->flash('success', trans("messages.admin_Manage_Contracts_Sports_Categories_Sports_category_has_been_updated_successfully"));
                return Redirect()->route("menu_setting.index");
            }
